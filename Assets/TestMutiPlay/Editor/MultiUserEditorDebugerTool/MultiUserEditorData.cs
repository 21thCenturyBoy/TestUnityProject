...
namespace TestMutiPlay
{
    ...
    public static class MultiUserEditorData
    {
        ...
        public class EditorStartup
        {
            ...
            static EditorStartup()
            {
                ...
                if (File.Exists(configPath))
                {
                    ...
                    m_Config = JsonUtility.FromJson<TemporaryConfig>(jsonStr);
                ...
                else
                {
                    ...
                    {
                        TemporaryConfig config = new TemporaryConfig();
                ...
                //{
                //    MultiUserEditorCommunication.PipeClientStartUp(ProjectName);
                //}
                //else
                //{
                //    MultiUserEditorCommunication.PipeServerStartUp();
                //}
            }

            //[UnityEditor.Callbacks.DidReloadScripts(0)]
            static void OnCallServer()
            {
                if (IsTemporaryProject)
                {
                    MultiUserEditorCommunication.PipeClientStartUp(ProjectName);
        ...
        [MenuItem("Tools/MultiUserTools/生成4个临时项目")]
        public static void Open4()
        {
        }

        [MenuItem("Tools/MultiUserTools/生成2个临时项目")]
        public static void Open2()
        {
        }

        [MenuItem("Tools/MultiUserTools/生成&启动1个临时项目")]
        ...
        private static void CreateTemporaryProject(string orginpath, string targetpath)
        {
            ...
            {
                string cmdline =
                    $"mklink /j \"{Path.Combine(targetpath, linkFloder[i])}\" \"{Path.Combine(orginpath, linkFloder[i])}\"";
                process.StandardInput.WriteLine(cmdline);
            }

            //TODO 
            //string[] copyFloder = new[] { "Logs" };
            //for (int i = 0; i < copyFloder.Length; i++)
            //{
            //    string cmdline = $"Xcopy \"{Path.Combine(orginpath, copyFloder[i])}\" \"{Path.Combine(targetpath, copyFloder[i])}\" /e /y /d";
            //    process.StandardInput.WriteLine(cmdline);
            //}
        ...
        {
            string path = EditorStartup.m_CommandLineArgs[2];
            ...
            if (!Directory.Exists(projectCopyFullPath))
            {
                Directory.CreateDirectory(projectCopyFullPath);
                TemporaryConfig config = new TemporaryConfig();
                config.Description = "多开缓存目录,硬盘不够可删！";
                config.OrginUnityProjectPath = path;
                config.OrginUnityProjectName = projectname;
                config.TempUnityProjectPaths = projectTempPaths;
                config.TempUnityProjectNames = projectTempNames;

                File.WriteAllText(Path.Combine(projectCopyFullPath, "ReadMeDontModify.json"),
                    JsonUtility.ToJson(config, true), Encoding.UTF8);
            }
